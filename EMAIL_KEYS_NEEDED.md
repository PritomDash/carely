...
`carely-backend/utils/emailService.js` now tries **Resend → Brevo → SendGrid** in order for every email. If one is unconfigured, down, or over its free-tier cap, it automatically falls through to the next — you don't need all three for email to work, but having all three gives you the most free capacity and the most redundancy (~15,000 emails/month combined, and no single point of failure).

You can add just one to start (Resend is simplest) and add the others later — nothing else needs to change in the code.
...
### 1. Resend (free: 3,000 emails/month, 100/day)
...
### 2. Brevo (free: 9,000 emails/month, 300/day)
...
5. Optional: **Senders** → add/verify a sender email, then set `BREVO_FROM_EMAIL` to that address (defaults to `noreply@carely.com` otherwise, which will fail Brevo's sender verification until you either verify that address or change it).
